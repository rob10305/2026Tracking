...
const PILLARS = [
  ...
  },
  {
    ...
    ring: "ring-slate-200",
    prefix: "Support and Ops",
    standardDeliverables: ["Support and Ops - Customer Onboarding"],
  },
];
...
export default function ProductLaunchPage() {
  ...
  const addActivity = useCallback(
    (productId: string, pillarPrefix: string, name: string) => {
      if (!name.trim()) return;
      const deliverable = `${pillarPrefix} - ${name.trim()}`;
      const reqs = getReqs(productId);
      if (reqs.some((r) => r.deliverable === deliverable)) return;

      const pillar = PILLARS.find((p) => p.prefix === pillarPrefix);
      if (!pillar) return;

      const lastIdx = reqs.reduce((acc, r, i) => {
        if (pillar.standardDeliverables.includes(r.deliverable) || r.deliverable.startsWith(pillarPrefix + " - ")) {
          return i;
        }
        return acc;
      }, -1);

      const newReq: LaunchRequirement = {
        deliverable,
        owner: "",
        criticalPath: "",
        timeline: "",
      ...
      const updated = [...reqs];
      updated.splice(lastIdx + 1, 0, newReq);
      updateLaunchRequirements(productId, updated);
    },
    [getReqs, updateLaunchRequirements],
  );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage launch activities for each product across all pillars. Add custom activities to tailor each product's launch plan.
          </p>
        </div>
              ...
              {isExpanded && (
                <div className="border-t border-gray-200 px-5 py-4 space-y-5">
                  {PILLARS.map((pillar, pillarIdx) => {
                    const pillarReqs = getReqsForPillar(reqs, pillar);
                    if (pillarReqs.length === 0 && !addingToPillar?.startsWith(`${p.id}::${pillar.id}`)) {
                      ...
                      return (
                        <div key={pillar.id}>
                          <div className="flex items-center gap-3 mb-3">
                            ...
                            <button
                              onClick={() => {
                                setAddingToPillar(`${p.id}::${pillar.id}`);
                                setNewActivityName("");
                              }}
                              className={`flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-md border ${pillar.border} ${pillar.text} ${pillar.bg} hover:opacity-80 transition-opacity`}
                            >
                              <Plus className="w-3.5 h-3.5" /> Add Activity
                            </button>
                          </div>
                          {addingToPillar === `${p.id}::${pillar.id}` && (
                            <AddActivityRow
                              pillar={pillar}
                              value={newActivityName}
                              onChange={setNewActivityName}
                              onAdd={() => {
                                addActivity(p.id, pillar.prefix, newActivityName);
                                setNewActivityName("");
                                setAddingToPillar(null);
                              }}
                              onCancel={() => setAddingToPillar(null)}
                            />
                          )}
                                  ...
                                  >
                                    <td className="px-4 py-2.5">
                                      <div className="flex items-center gap-2">
                                        ...
                                        {isCustom && (
                                          <span className="text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-indigo-100 text-indigo-600 border border-indigo-200">
                                            CUSTOM
                                          </span>
                                        )}
                                        {hasDependants && (
                                    ...
                                    <td className="px-4 py-2.5">
                                      <select
                                        value={r.dependency || ""}
                                        onChange={(e) =>
                                          updateField(p.id, r.deliverable, "dependency", e.target.value)
                                        }
                                        className={`w-full text-xs border rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-400 ${
                                          r.dependency
                                    ...
                                    <td className="px-4 py-2.5 text-center">
                                      {isCustom && (
                                        <button
                                          onClick={() => removeActivity(p.id, r.deliverable)}
...
function AddActivityRow({
  ...
  return (
    <div className={`mt-2 flex items-center gap-2 px-4 py-2 rounded-lg border ${pillar.border} ${pillar.bg}`}>
      <Plus className={`w-4 h-4 ${pillar.text} flex-shrink-0`} />
      <span className={`text-xs font-medium ${pillar.text} flex-shrink-0`}>{pillar.prefix} -</span>
      <input
        autoFocus
        value={value}
      ...
      <button
        onClick={onAdd}
        disabled={!value.trim()}
        className={`text-xs font-medium px-3 py-1 rounded ${pillar.accent} text-white disabled:opacity-40`}
      >
        Add
      </button>
      <button
