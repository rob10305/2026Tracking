...
- **Sales Motion Tracker:** Full port of the standalone `sales-motion-tracker` GitHub app into the Next.js app at `/sales-motion`. Inner layout with left sidebar (motions list) and user switcher (Jaime, Daniel G, Mike, Shane, Daniel R). Three views: Dashboard (motion cards with goal/actual/variance/RAG), MotionDetail (activity tracker with category/task rows), and Monthly KPIs grid. **Data persisted in PostgreSQL** via `SalesMotionTrackerState` model (single JSON row, id=1). API: `GET/PUT /api/sales-motion/state`. Context loads from DB on mount, saves with 1500ms debounce on every state change. Source files: `lib/sales-motion/` (types, utils, data, context), `components/sales-motion/` (all UI components).
