...
type MbrKind = "wins" | "misses" | "risks" | "asks";

type Item = { id?: string; kind: MbrKind; order: number; body: string };

const QUADRANTS: Array<{ key: MbrKind; label: string; accent: AccentKey }> = [
  { key: "wins", label: "Wins", accent: "emerald" },
  { key: "misses", label: "Misses", accent: "amber" },
  { key: "risks", label: "Risks", accent: "sky" },
  { key: "asks", label: "Asks", accent: "violet" },
];

export default function EditableMbrHighlights({
  dept,
  config,
}: {
  dept: string;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/aop/${dept}/mbr-highlights`, { cache: "no-store" });
        ...
        if (loaded.length === 0) {
          ...
          for (const q of QUADRANTS) {
            const source = config.mbrHighlights[q.key] ?? [];
        ...
        console.error("Failed to load mbr highlights", e);
      } finally {
  ...
  const save = async () => {
    setSaving(true);
    try {
      const res = await fetch(`/api/aop/${dept}/mbr-highlights`, {
        method: "PUT",
      ...
      const r2 = await fetch(`/api/aop/${dept}/mbr-highlights`, { cache: "no-store" });
      ...
      console.error("Failed to save mbr highlights", e);
      const msg = e instanceof Error ? e.message : "Unknown error";
      alert(`Failed to save MBR highlights.\n\n${msg}`);
    } finally {
      setSaving(false);
    }
  };

  const itemsFor = (kind: MbrKind) =>
    (editing ? draft : items)
      .filter((i) => i.kind === kind)
      .sort((a, b) => a.order - b.order);

  const addItem = (kind: MbrKind) => {
    setDraft((prev) => {
  ...
  const updateItem = (kind: MbrKind, kindIdx: number, body: string) => {
    setDraft((prev) => {
      const result: Item[] = [];
      let counter = 0;
  ...
  const removeItem = (kind: MbrKind, kindIdx: number) => {
    setDraft((prev) => {
      const result: Item[] = [];
      let counter = 0;
      for (const item of prev) {
